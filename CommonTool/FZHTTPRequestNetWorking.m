//
//  FZHTTPRequestNetWorking.m
//  CommonTool
//
//  Created by 方正 on 2016/12/28.
...
@implementation FZHTTPRequestNetWorking
{
    AFHTTPSessionManager *_HTTPManager;
}
+ (id)sharedInstance
{
    static FZHTTPRequestNetWorking * request;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        request = [[FZHTTPRequestNetWorking alloc] init];
    });
    return request;
}
              ...
              showHUD:(UIView *)showView
{
    ...
    [self createUnloginedRequest:taskID WithParam:param withMethod:method success:success failure:failure showHUD:showView];
    
}

- (void)createUnloginedRequest:(NSString*)taskID WithParam:(NSDictionary *)param withMethod:(HTTPRequestMethod)method success:(void(^)(id result))success failure:(void(^)(NSError *erro))failure showHUD:(UIView *)showView
{
    ...
    //    NSString * url = [NSString stringWithUTF8String:taskID];
    NSString *URLString = [NSString stringWithFormat:@"%@%@",self.baseApi,taskID];
    ...
    NSURLSessionDataTask *dataTask = [_HTTPManager dataTaskWithRequest:request completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        if (error) {
            ...
            if (success != nil)
            {
                ...
                NSArray *cookies = [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookiesForURL: [NSURL URLWithString:self.baseApi]];
...
- (void)createUploadRequest:(NSString*)taskID
            WithParam:(NSDictionary*)param
...

{
    ...
    [self createUnloginedRequest:taskID WithParam:param withExParam:Exparam withMethod:method success:success failure:failure uploadFileProgress:uploadFileProgress];
...
{
    if (URLString) {
        [self createUnloginedDownloadFileWithURLString:URLString downloadFileProgress:downloadFileProgress setupFilePath:setupFilePath downloadCompletionHandler:downloadCompletionHandler];
    }
}
