    ...
    if ([self dzn_shouldDisplay] && [self dzn_itemsCount] <= 0)
    {
        ...
        if (customView) {
            ...
            if (buttonImage) {
                ...
                UIImage *normalBG = [self dzn_buttonBackgroundImageForState:UIControlStateNormal];
                [view.button setAttributedTitle:[self dzn_buttonTitleForState:UIControlStateHighlighted] forState:UIControlStateHighlighted];
                [view.button setBackgroundImage:normalBG forState:UIControlStateNormal];
                ...
                if (normalBG) {
                    view.button.frame = CGRectMake(view.center.x, view.button.frame.origin.y, normalBG.size.width, normalBG.size.height);
                }
            }
...
- (void)updateConstraints
{
    ...
    if ([self canShowButton]) {
        ...
        NSNumber *btnTrailing = @(roundf((width- btnWidth)/2.0));
        if (!btnWidth) {
            btnTrailing = padding;
        }
        NSDictionary *metrics = NSDictionaryOfVariableBindings(btnTrailing);
        [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-btnTrailing-[button]-btnTrailing-|"
                                                                                 options:0 metrics:metrics views:views]];
    }
    // or removes from its superview
    else {
